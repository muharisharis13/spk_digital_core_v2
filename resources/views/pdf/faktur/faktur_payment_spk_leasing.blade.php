...
<body style="padding: 0; margin: 0; font-family: Arial, Helvetica, sans-serif;">
    <div class="row" style="height: 200px;">
        ...
        <div class="right" style="text-align: right">
            <h2 style="text-transform: uppercase">KWITANSI SPK {{ $data->data_leasing->spk_payment_type }}</h2>
            <div>No.{{ $data->data_leasing->spk_payment_number }}</div>
            <div style="text-align: right;margin-top:10px">
                <img src="data:image/png;base64, {!! base64_encode(QrCode::size(100)->generate(url("/detail/payment/$data->spk_payment_id"))) !!} ">
                {{-- {!!  !!} --}}

            </div>
            <div style="margin-top: 10px;font-weight:bold">
                {{ date('d M Y', strtotime($data->data_leasing->created_at)) }}
            </div>
        </div>
    </div>
    ...
    <table style="margin-top: 10px;width:100%">
        <tr>
            ...
            <td>:</td>
            <td>{{ $data->spk->spk_transaction->leasing_name }}</td>
        </tr>
        <tr>
            <th style="space-white:nowrap;padding-bottom:10px">
                <div style="white-space: nowrap">Nama Pembeli</div>
            </th>
            <td>:</td>
        ...
        <tr>
            <th style="space-white:nowrap;padding-bottom:10px">
                <div style="white-space: nowrap">Nama STNK</div>
            </th>
            <td>:</td>
            <td>{{ $data->spk->spk_legal->spk_legal_name }}</td>
        </tr>
        <tr>
        ...
        <tr>
            <th style="padding-bottom:10px">Rangka / Mesin</th>
            <td>:</td>
            <td>{{ $data->data_leasing->spk->spk_unit->unit->unit_frame }} /
                {{ $data->data_leasing->spk->spk_unit->unit->unit_engine }}</td>
        </tr>
        <tr>
            <th>Keterangan</th>
            <td>:</td>
    ...
    <div>
        <div class="row" style="height: 200px;">
            ...
            <div class="right" style="text-align: right">
                <h2 style="text-transform: uppercase">KWITANSI SPK {{ $data->data_dp->spk_payment_type }}</h2>
                <div>No.{{ $data->data_dp->spk_payment_number }}</div>
                <div style="text-align: right;margin-top:10px">
                ...
                <div style="margin-top: 10px;font-weight:bold">
                    {{ date('d M Y', strtotime($data->data_dp->created_at)) }}
                </div>
            </div>
        </div>
        ...
        <table style="margin-top: 10px;width:100%">
            <tr>
                ...
                <td>:</td>
                <td>{{ $data->data_dp->spk->spk_customer->spk_customer_name }}</td>
            </tr>
            <tr>
                <th style="padding-bottom:10px">Banyaknya Uang</th>
                <td>:</td>
                <td> Rp {{ number_format($data->spk_payment_amount_total_dp, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <th style="padding-bottom:10px">
                    <div style="white-space: nowrap">Untuk Pembayaran</div>
            ...
            <tr>
                <th style="padding-bottom:10px">Type</th>
                <td>:</td>
                <td>{{ $data->data_dp->spk->spk_unit->motor->motor_name }}</td>
            </tr>
            <tr>
                <th style="padding-bottom:10px">Warna</th>
                <td>:</td>
                <td>{{ $data->data_dp->spk->spk_unit->color->color_name }}</td>
            </tr>
            <tr>
                <th style="padding-bottom:10px">Rangka / Mesin</th>
                <td>:</td>
                <td>{{ $data->data_dp->spk->spk_unit->unit->unit_frame }} /
                    {{ $data->data_dp->spk->spk_unit->unit->unit_engine }}</td>
            </tr>
            <tr>
                <th>Keterangan</th>
                <td>:</td>
    ...
    margin-top:40px;margin-bottom:40px">
        ...
        <div style="margin-top:40px">
            <table class="table" style="width: 100%">
                ...
                @if ($data->data_dp->spk_payment_list->count() > 0)

                    <tbody>
                        @foreach ($data->data_dp->spk_payment_list as $item)
                            <tr>
                                <td class="td" style="text-transform: uppercase">
                                    {{ $item->spk_payment_list_method }}
                                </td>
                                <td class="td">{{ $item->bank->bank_name ?? '-' }}</td>
                                <td class="td">{{ number_format($item->spk_payment_list_amount, 0, ',', '.') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    margin-top:40px;margin-bottom:40px">
            <div style="width:100%;">
                <div
                    style="float:left; text-align:center; border:1px solid black; width:50%; padding:10px; border-radius:5px;">
                    <strong>Jumlah Rp. {{ number_format($data->spk_payment_amount_total_dp, 0, ',', '.') }}</strong>
                </div>
                <div style="padding:10px;text-align:center;white-space:nowrap">
                    Lembar 1 : Poskas
                </div>
